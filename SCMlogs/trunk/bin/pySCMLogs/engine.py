...
class SCMconfig:
	...
	def load_options (self, cfg):
		# Global
		ls = "global"
		self.data_dir = self.option (cfg, "global", "data_dir")
		...
		self.mySCMlogs_url = self.option (cfg, "global", "SCMlogs_url")
		# SCM mode
		self.SCMmode = self.option (cfg, "global", "SCMmode")
		self.repository_path = self.option (cfg, self.SCMmode, "repository_path")
		self.repository_name = self.option (cfg, self.SCMmode, "repository_name")
		self.logs_dir = self.option (cfg, self.SCMmode, "logs_dir")
		# Browsing
		if cfg.has_option ("global", "browsing"):
			self.browsing = self.option (cfg, "global", "browsing")
			self.webapp_url = self.option (cfg, self.browsing, "webapp_url")
		else:
			self.browsing = ""
			self.webapp_url = "%s%s" % (self.mySCMlogs_url, "webscmlogs.php")
		# Email
...
#			self.webUrlEngine.set_default_webapp ();
	...
	#		print "ARG = %s" %(arg)
			if arg == '-f':
				...
				i = i + 1
				self.only_user = argv[i]
			elif arg == '-only_tag':
				i = i + 1
				self.only_tag = argv[i]
			elif arg == '-mesg':
				i = i + 1
				self.opt_mesg = argv[i]
			elif arg == '-subject':
				i = i + 1
				self.opt_subject = argv[i]
			elif arg == '-cfg':
				i = i + 1
				self.opt_cfg = argv[i]
			elif arg == '-html':
				self.opt_output_format = 'html'
			elif arg == '-text':
				self.opt_output_format = 'text'
			elif arg == '-out':
				self.opt_output = 'out'
			elif arg == '-mail':
				self.opt_output = 'mail'
	...
	def load_config (self):
		self.config= SCMconfig(self.opt_cfg)
...
def processMain():
	app = SCMLogsApplication(sys.argv);
	app.execute()
